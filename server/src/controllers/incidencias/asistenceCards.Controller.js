...
asistenceCardsController.printSingleAsistenceCard = async (req, res) => {
  const docWidth = 8.1 * 28.35; // Convert cm to points (1 cm = 28.35 points)
  const docHeight = 18.3 * 28.35;
  const doc = new PDFDocument({ size: [docWidth, docHeight] });
  doc.fontSize(9); // Increased font size by 1 point

  try {
    const { numtar } = req.params;
    const numtarInt = parseInt(numtar, 10);
    if (!numtar) {
      return res
        .status(400)
        .json({ message: "El parámetro NUMTAR es obligatorio." });
    }

    const tarjeta = await query("PLANTILLA", { NUMTARJETA: numtarInt });
    console.log(tarjeta);

    if (!tarjeta || tarjeta.length === 0) {
      return res.status(404).json({
        message: "No se encontró un registro para el NUMTAR proporcionado.",
      });
    }

    const record = tarjeta[0];
    // Add text for the record
    doc.fontSize(18).font("Helvetica-Bold"); // Increased font size for NUMTAR by 3 points
    doc.text(record.NUMTARJETA, (7 - 0.2) * 28.35, (1.8 - 1 - 0.2) * 28.35);
    doc.fontSize(8.5).font("Helvetica"); // Reset font size and style for other text
    ...
    const filePath = path.join(__dirname, "asistenceCard.pdf");
    const stream = fs.createWriteStream(filePath);
    ...
    stream.on("finish", () => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=asistenceCard.pdf`
      );
      const fullName = `${record.APE_PAT} ${record.APE_MAT} ${record.NOMBRES}`;
      const fileName = `TARJETA_${fullName}.pdf`;
    ...
    res.status(500).json({
      message: "Error al generar la tarjeta de asistencia.",
      details: error.message,
    });
  }
};
